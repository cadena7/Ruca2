'''
LIBRERIA ROBOCLAW PARA PYTHON 3.5
Version 1.2        19/Jun/2018
...
Funciones Añadidas:
Ver. 1.2 - Se agrego la funcion de leer valores negativos del encoder, leer documentacion de la libreria struct: https://docs.python.org/3/library/struct.html#format-characters
Ver. 1.1 - Modificada para Python 3 debido a cambios en la libreria serial (se apoyo en un fork que realizó alguien en internet)
Ver. 1.0 - Versión original dada por el fabricante para Python 2.7
'''
...
class RoboClaw:
    ...
    def drive_to_position_raw(self, motor, accel, speed, deccel, position, buffer):
        # drive to a position expressed as pulses of the encoder
        if motor == 1:
...
# MUEVE MOTOR HACIA ATRAS
    ...
    def read_encoder(self, motor):
        ...
        if motor == 1:
            ...
            cmd = Cmd.GETM2ENC
        #return self._read(cmd, '>IB')[0]      # Original unsigned
        return self._read(cmd, '>iB')[0]       # Para leer valores negativos signed --> https://docs.python.org/3/library/struct.html

    def reset_quad_encoders(self):
        self._write(Cmd.SETM1ENCCOUNT, '>I', 0)
        self._write(Cmd.SETM2ENCCOUNT, '>I', 0)

        # Agregue esta funcion para resetear solo el encoder deseado con la interrupcion del switch limite
    def reset_quad_encoder(self, encoderdeseado):
        if encoderdeseado == 1:
            self._write(Cmd.SETM1ENCCOUNT, '>I', 0)
        else:
            self._write(Cmd.SETM2ENCCOUNT, '>I', 0)
